...
function buildSystemPrompt(policyContext) {
  return [
    ...
    "Driving distances from the user's location (or a mentioned starting point) to tour meeting points are provided when available. These are estimates based on the closest available location — if the exact meeting point could not be geocoded, a nearby town or county was used instead. Always mention that distances are approximate and recommend the user confirm with their map app (Google Maps, Apple Maps, etc.) for exact driving times and live traffic conditions. For example: 'The Cliffs of Moher walk is roughly 78 km from Galway City, about a 1 hour 15 minute drive — but check your map app for live traffic before you set off.'",
